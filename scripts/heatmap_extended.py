...
# Load CSV data
csv_file = "/home/maximilian/CLionProjects/MolSim/scripts/output/2025-01-25 16:40:42/density.csv"  # Replace with your CSV file path
output_directory = "./output/heatmap_extended_" + datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
...
if len(data.columns) < 226:
    raise ValueError("The CSV file must have at least 225 columns: 'time' and 225 data columns.")

# Extract time and data columns
time = data.iloc[:, 0]  # First column is time
data_columns = data.iloc[:, 1:256]  # Extract the next 50 columns
...
for i, row in enumerate(data_array):
    ...
    ax.text(3, 0.7, f"min: {current_min:04.2f}, avg: {current_avg:04.2f}, max: {current_max:04.2f}")
