...
Particle::Particle(const std::array<double, 3>& x_arg, const std::array<double, 3>& v_arg,
                   const double m_arg, const double sigma, const double epsilon, const int type) : x(x_arg), v(v_arg), f{0.,0.,0.}, old_f
  {0.,0.,0.}, m(m_arg), type(type), sigma(sigma), epsilon(epsilon) {
  DEBUG_PRINT("Particle generated!");
}
