...
/**
 * @brief Interface for an object storing particles while providing single and pairwise iterators.
 */
...
public:
    ...
    /**
     * @brief Get a vector containing references to all particles in the container.
    ...
    /**
     * @brief Apply a function to each unique pair of particles (i.e., (p1, p2) == (p2, p1)).
     * @param f Lambda to be applied to each particle pair (p1, p2).
     */
    virtual void pairIterator(const std::function<void(Particle&, Particle&)>& f) = 0;
};
