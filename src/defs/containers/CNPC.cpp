...
const double CNPC::sigma_factor = std::pow(2.0, 1.0 / 6.0);
...
CNPC::CNPC(
  ...
  for (std::size_t cell_index = 0; cell_index < cells.size(); ++cell_index) {
    ...
    auto cellCoordinate = cellIndexToCoord(cell_index);
    for (auto &offset : neighbours_offsets) {
      ...
      if (!isValidCellCoordinate(neighbourCoord)) {
        DEBUG_PRINT_FMT("Invalid coord: ({}, {}, {}) or cell_index is halo", neighbourCoord[0],
      ...
      neighbours.emplace_back(neighbourIndex);
    }
    ...
    neighbour_cells.emplace_back(cell_index, neighbours);
  }
...
void CNPC::addParticle(const Particle &p) {
...
void CNPC::removeParticle(const Particle &p) {
  const std::size_t index = dvec3ToCellIndex(p.getX());
...
std::vector<Particle *> CNPC::getParticles() {
...
std::vector<Particle> CNPC::getParticlesObjects() {
  std::vector<Particle> res;
  singleIterator([&res](const Particle &p) { res.push_back(p); });
  return res;
}

[[nodiscard]] std::size_t CNPC::size() const {
  std::size_t count = 0;
  for (auto &c : cells) {
    count += c.size();
  }
...
void CNPC::pairIterator(
  ...
  for (std::size_t cellIndex = 0; cellIndex < cells.size(); cellIndex++) {
    ...
    auto neighbours = std::get<1>(neighbour_cells[cellIndex]);
    for (const auto neighbour : neighbours) {
      ...
      std::vector<Particle> &neighbourParticles = cells[neighbour];
...
inline std::size_t CNPC::dvec3ToCellIndex(
...
inline bool CNPC::isValidCellCoordinate(
...
inline bool CNPC::isHalo(const ivec3 cellCoord) const {
...
inline bool CNPC::isHalo(const std::size_t cellIndex) const {
  const ivec3 cellCoord = cellIndexToCoord(cellIndex);
  return isHalo(cellCoord);
}

inline bool CNPC::isBoundary(const ivec3 cellCoord) const {
...
inline bool CNPC::isBoundary(
    const std::size_t cellIndex) const {
  const ivec3 cellCoord = cellIndexToCoord(cellIndex);
  return isBoundary(cellCoord);
}

std::vector<std::size_t> CNPC::special_cell_direction(
...
bool CNPC::isBoundary_testing(
    const std::size_t cellIndex) const {
  return isBoundary(cellIndex);
}

bool CNPC::isHalo_testing(const std::size_t cellIndex) const {
  return isHalo(cellIndex);
}

std::size_t CNPC::dvec3ToCellIndex_testing(
    const dvec3 &position) const {
  return dvec3ToCellIndex(position);
}

void CNPC::apply_reflective_boundary(const size_t dimension) {
  const std::size_t problematic_dimension = dimension / 2;
  const std::size_t problematic_dimension_direction = dimension % 2;
...
std::tuple<bool, ivec3, dvec3> CNPC::reflective_warp_around(
...
CNPC::reflective_warp_around_testing(
    const ivec3 cell_coordinate, const std::size_t raw_dimension) const {
  return reflective_warp_around(cell_coordinate, raw_dimension);
}

std::size_t CNPC::cellCoordToIndex_testing(
    const ivec3 position) const {
  return cellCoordToIndex(position);
}

ivec3 CNPC::cellIndexToCoord_testing(
    const std::size_t cellIndex) const {
  return cellIndexToCoord(cellIndex);
}

double CNPC::getKineticEnergy() {
  double E_kin = 0.0;
