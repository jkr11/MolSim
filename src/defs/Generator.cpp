...
ParticleGenerator::ParticleGenerator(
    ...
    : corner(corner), dimensions(dimensions), h(h), m(m),
      initialVelocity(initialVelocity), temp(temperature), type(type) {}

void ParticleGenerator::generate(ParticleContainer &container) {
  std::vector<Particle> particles = container.getParticles();
  particles.reserve(static_cast<int>(dimensions[0]*dimensions[1]*dimensions[2]));
